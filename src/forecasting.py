...
class DensityForecaster:
    ...
    def fit(self, Y_train, Y_support):
        ...
        self.model_lqd = mLQDT(Y_train, Y_support)
        ...
        self.kdfpc_kwargs.update({
            "u": lqd_support,
            "du": lqd_support[1] - lqd_support[0]
        })
        
        self.model_kdfpc = K_dFPC(lqd_values)
    ...
    def predict(self, horizon, var_lags=None):
        ...
        df_k_forecast = pd.DataFrame(k_curve_forecast)
        
        df_supports, df_densities = obtain_densities_from_lqd(
                                        df_k_forecast,
                                        self.model_lqd.lqd_support,
                                        c_forecast,
                                        t_=self.model_lqd.t,
                                        verbose=False
                                    )
