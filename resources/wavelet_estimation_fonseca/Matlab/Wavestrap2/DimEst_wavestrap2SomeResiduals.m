function [ vd0p1_boot ] = DimEst_wavestrap2SomeResiduals( Y, Nboot, B, p, N, wname)
...
% Nesse código somente alguns dos coeficientes limiarizados são usados nesse
% procedimento, nos níveis que tiveram pelo menos 33 coeficientes limiarizados.
...
for ii = 1:n
    ...
    [~,Aths(:,ii),~] = wden(Epshat(:,ii),'sqtwolog','h','sln',N,wname);
    ...
    cont = 1;  % contador
    for k=1:N
        if (sum(ind_ths((cumsum_Lw(k)+1):cumsum_Lw(k+1),ii))==0)
            ind_ths((cumsum_Lw(k)+1),ii) = 2;
            medj(k,ii) = 0;
            sigj(k,ii) = 0;
            numj(k,ii) = 1;
            r(cont:sum(numj(:,ii)), ii) = 0;
        else
...
for k=1:N
    ...
    % k
    if (sum(ind_ths((cumsum_Lw(k)+1):cumsum_Lw(k+1),ts))<33)
        if (ind_ths((cumsum_Lw(k)+1),ts)~=2)
            Aboot(find(ind_ths((cumsum_Lw(k)+1):cumsum_Lw(k+1),ts))+cumsum_Lw(k),jj) = Aboot(find(ind_ths((cumsum_Lw(k)+1):cumsum_Lw(k+1),ts))+cumsum_Lw(k),jj) + A_res(find(ind_ths((cumsum_Lw(k)+1):cumsum_Lw(k+1),ts))+cumsum_Lw(k),ts);
        end
    else
