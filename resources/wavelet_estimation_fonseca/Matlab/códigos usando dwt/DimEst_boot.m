function [ vd0p1_boot ] = DimEst_boot( Y, NREP, B, p, wname)
...
mu_hat =  idwt(mu_A(1:(J/2)),mu_A((J/2 + 1):end),wname);

for ii=1:d0
    H(:,ii) = idwt(B(1:(J/2),ii),B((J/2 + 1):end,ii),wname);
end
...
for ii=1:n
    Yboot(:,ii) = Yhat(:,ii) + datasample(mEps_hat(:,ii),nt);
    [D,L] = dwt(Yboot(:,ii),wname);
    Aboot(:,ii) = [D',L']';
end
